...
function ResultBox({ label, amount, indigo }) {
  return (
    <div className={`mx-4 flex justify-between items-center rounded-lg px-3 py-2 mt-1.5 ${indigo ? 'bg-indigo-50 border border-indigo-100' : 'bg-gray-100 border border-gray-200'}`}>
      <span className={indigo ? 'text-indigo-700 text-xs' : 'text-gray-500 text-xs'}>= {label}</span>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-md shadow-gray-100 overflow-hidden">
      ...
      <div className="px-4 py-2.5 border-b border-gray-100 flex items-center justify-between gap-2">
        <div className="flex rounded-full border border-gray-200 bg-gray-50 p-0.5 gap-0.5">
          ...
          >
            New Regime
            {computeSuccess && betterRegime === 'new' && <span className="ml-1 text-green-600 text-[10px] font-bold">Best</span>}
          </button>
          <button 
          ...
          >
            Old Regime
            {computeSuccess && betterRegime === 'old' && <span className="ml-1 text-green-600 text-[10px] font-bold">Best</span>}
          </button>
        </div>
        {userPickedRegime !== null && (
          <button onClick={() => setUserPickedRegime(null)} className="text-[11px] text-indigo-500 hover:text-indigo-700 underline decoration-dotted underline-offset-2 shrink-0">
            Auto-select best
          </button>
        )}
      </div>
      ...
      {!hasIncome ? (
        <div className="px-4 py-8 text-center flex flex-col items-center">
          <div className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center mb-3">
            <svg className="w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
          </div>
        ...
        <div className="space-y-1 pb-3">
          {computeSuccess && (
            <div className="mx-4 mt-3 rounded-xl bg-gradient-to-br from-indigo-600 via-indigo-700 to-purple-700 p-4 text-white">
              ...
              <p className="text-xs text-indigo-300 mt-1">On annual income of {fmt(activeData.grossIncome)}</p>
            </div>
          )}
          ...
          {regime === 'new' && activeData.employerNPSDeduction === 0 && (
            <div className="mx-4 flex items-start gap-1.5 mt-1.5 p-2 bg-gray-50 rounded-lg">
              <svg className="w-3 h-3 text-gray-400 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          ...
          <div className="mx-4 bg-gray-50 rounded-lg overflow-hidden border border-gray-100 mb-2">
          ...
          <div className="mx-4 mt-2 flex justify-between items-center bg-indigo-600 rounded-xl px-3 py-2.5">
      ...
      <div className="flex items-center justify-center gap-2 px-4 py-3 border-t border-gray-100 bg-gray-50/50 mt-auto">
